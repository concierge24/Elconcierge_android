    ...
    private fun validateResponse(it: CategoryListModel?) {
        ...
        when (it?.status) {
            NetworkConstants.SUCCESS -> {
                ...
                val updatedData = it.data

                if (dataManager.getKeyValue(PrefenceConstants.CATEGORY_ID, PrefenceConstants.TYPE_STRING).toString().isNotEmpty()) {
                    val catId = dataManager.getKeyValue(PrefenceConstants.CATEGORY_ID, PrefenceConstants.TYPE_STRING).toString()
                    updatedData?.brands=updatedData?.brands?.filter { it.cat_id == catId }
                }
                homeDataLiveData.value = updatedData
            }
            NetworkConstants.AUTHFAILED -> {
                dataManager.setUserAsLoggedOut()
                navigator.onSessionExpire()
